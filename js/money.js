// public/js/money.js - النسخة الكاملة والنهائية (مع إصلاح كل المشاكل)

document.addEventListener('DOMContentLoaded', () => {
    // لا يتم استدعاء buildSidebar هنا لأنه يتم استدعاؤه من main.js
    loadPageData();
    setupEventListeners();
    updateClock();
...
async function loadPageData() {
    ...
    try {
        const [shiftData, historyDays] = await Promise.all([
            sendRequest('/api/shifts/active', 'GET'),
            currentUser?.role === 'admin' ? sendRequest('/api/shifts/history/days', 'GET') : Promise.resolve([])
        ]);
...
function updateUI(reportData = null) {
    ...
    if (dataProvider) {
        ...
        ['netCashValue', 'totalExpensesValue', 'totalRevenuesValue', 'ordersRevenue', 'tablesRevenue', 'devicesRevenue'].forEach(id => document.getElementById(id).textContent = '0.00 ج');
...
function handleShiftControl() { if (activeShift) openModal('endShiftModal'); else { const form = document.getElementById('startShiftForm'); form.reset(); openModal('startShiftModal'); } }
async function startShift() { const form = document.getElementById('startShiftForm'); await handleRequest(sendRequest('/api/shifts/start', 'POST', { starting_cash: form.startingCash.value }), 'تم بدء الشيفت بنجاح!', () => { closeModal('startShiftModal'); loadPageData(); }); }
async function endShift() { const form = document.getElementById('endShiftForm'); await handleRequest(sendRequest('/api/shifts/end', 'POST', { ending_cash: form.endingCash.value, notes: form.shiftNotes.value }), 'تم إنهاء الشيفت بنجاح!', () => { closeModal('endShiftModal'); loadPageData(); }); }
function openTransactionModal(type) { const form = document.getElementById('addTransactionForm'); form.reset(); document.getElementById('transactionType').value = type; const title = document.getElementById('transactionModalTitle'); title.innerHTML = (type === 'revenue') ? '<i class="fas fa-arrow-up"></i> إضافة إيراد' : '<i class="fas fa-arrow-down"></i> إضافة مصروف'; openModal('addTransactionModal'); }
async function saveTransaction() { const form = document.getElementById('addTransactionForm'); const data = { type: form.transactionType.value, description: form.transactionDescription.value, amount: form.transactionAmount.value, category: form.transactionCategory.value }; await handleRequest(sendRequest('/api/transactions', 'POST', data), 'تم تسجيل المعاملة', loadActiveShift); closeModal('addTransactionModal'); }
async function saveNote() { showNotification('قيد التطوير', 'info'); closeModal('addNoteModal'); }
function updateClock() { document.getElementById('current-date-time').textContent = new Date().toLocaleDateString('ar-EG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }); }
function toggleFabOptions() { document.querySelector('.fab-container')?.classList.toggle('active'); }
function populateHistoryDropdown(days) { const select = document.getElementById('daysHistorySelect'); const container = select?.closest('.dropdown-container'); if (!select || !container) return; if (currentUser?.role === 'admin' && days?.length > 0) { container.style.display = 'flex'; select.innerHTML = '<option value="today">عرض اليوم الحالي</option>'; days.forEach(day => select.innerHTML += `<option value="${day}">${day}</option>`); } else { container.style.display = 'none'; } }
function handleHistorySelection(event) { const selectedDay = event.target.value; switchTab('revenue'); if (selectedDay === 'today') { loadPageData(); } else { loadReportForDay(selectedDay); } }
async function loadReportForDay(date) { showNotification(`جاري تحميل تقرير يوم ${date}...`, 'info'); try { const shiftsOfDay = await sendRequest('/api/shifts/report/day', 'POST', { date }); let aggregatedData = { date, total_revenues: 0, total_expenses: 0, starting_cash: 0, notes: [], revenues: [], expenses: [] }; shiftsOfDay.forEach(shift => { const shiftRevenues = shift.transactions.filter(t => t.type === 'revenue'); const shiftExpenses = shift.transactions.filter(t => t.type === 'expense'); aggregatedData.total_revenues += shiftRevenues.reduce((sum, t) => sum + parseFloat(t.amount), 0); aggregatedData.total_expenses += shiftExpenses.reduce((sum, t) => sum + parseFloat(t.amount), 0); aggregatedData.starting_cash += parseFloat(shift.starting_cash); if (shift.notes) aggregatedData.notes.push(shift.notes); aggregatedData.revenues.push(...shiftRevenues); aggregatedData.expenses.push(...shiftExpenses); }); aggregatedData.net_cash = aggregatedData.starting_cash + aggregatedData.total_revenues - aggregatedData.total_expenses; aggregatedData.notes = aggregatedData.notes.join(' | ') || 'لا توجد ملاحظات لهذا اليوم.'; updateUI(aggregatedData); } catch (error) { showNotification('فشل تحميل التقرير', 'error'); } }
